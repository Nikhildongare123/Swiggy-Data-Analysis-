# 🍽️ Swiggy Bangalore Data Analysis

<div align="center">

# 📊 Swiggy Bangalore Data Analysis using Python

Analyze Swiggy restaurant data to uncover insights about ratings, cuisines, locations, pricing, and customer preferences.

<img src="Notebook/displot.png" width="700">

</div>

---

# 📌 Project Overview

This project performs Exploratory Data Analysis (EDA) on Swiggy Bangalore restaurant data using Python. The objective is to identify customer preferences, popular cuisines, pricing trends, restaurant ratings, and location-wise insights through visualizations.

The project demonstrates practical skills in:

- Data Cleaning
- Data Analysis
- Data Visualization
- Business Insights
- Exploratory Data Analysis (EDA)

---

# 📂 Dataset

Dataset contains restaurant information including:

- Restaurant Name
- Cuisine
- Location
- Rating
- Cost for Two

---

# 🛠️ Tech Stack

- Python
- Pandas
- NumPy
- Matplotlib
- Seaborn
- Jupyter Notebook

---

# 📁 Project Structure

```
Swiggy-Data-Analysis/
│
├── Data/
│   └── Swiggy Bangalore Outlet Details.csv
│
├── Notebook/
│   ├── swiggy-analysis.ipynb
│   └── displot.png
│
├── README.md
└── requirements.txt
```

---

# 📊 Analysis Performed

### ✅ Data Cleaning

- Removed missing values
- Checked duplicate records
- Converted data types
- Cleaned Cost for Two column

---

### ✅ Exploratory Data Analysis

- Restaurant Distribution
- Cuisine Analysis
- Rating Distribution
- Cost Analysis
- Location Analysis
- Top Rated Restaurants
- Most Expensive Restaurants
- Budget Friendly Restaurants

---

# 📈 Visualizations

The project includes:

- Bar Charts
- Count Plots
- Histograms
- Box Plots
- Pie Charts
- Scatter Plots
- Distribution Plots
- Heatmaps

---

# 📌 Key Insights

- Identified highest-rated restaurants.
- Found the most popular cuisines.
- Compared restaurant prices across locations.
- Discovered affordable restaurants with high ratings.
- Analyzed rating distribution.
- Identified premium restaurant locations.

---

# ▶️ How to Run

### Clone Repository

```bash
git clone https://github.com/Nikhildongare123/Swiggy-Data-Analysis-.git
```

### Open Project

```bash
cd Swiggy-Data-Analysis-
```

### Install Libraries

```bash
pip install -r requirements.txt
```

### Run Notebook

```bash
jupyter notebook
```

Open

```
Notebook/swiggy-analysis.ipynb
```

---

# 📷 Sample Output

Restaurant Analysis

- Ratings Distribution
- Cuisine Frequency
- Cost Analysis
- Top Restaurants

---

# 📚 Python Libraries Used

```python
import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
import seaborn as sns
```

---

# 🎯 Business Insights

This project helps answer questions like:

- Which locations have the highest-rated restaurants?
- Which cuisines are most popular?
- What is the average cost for two?
- Which restaurants offer the best value for money?
- Which areas contain premium restaurants?

---

# 🚀 Future Improvements

- SQL Integration
- Power BI Dashboard
- Machine Learning
- Restaurant Recommendation System
- Streamlit Dashboard
- Flask Deployment

---

# 👨‍💻 Author

**Nikhil Dongare**

- GitHub: https://github.com/Nikhildongare123
- LinkedIn: *(Add your LinkedIn profile here)*

---

# ⭐ If you found this project useful, please give it a Star!
